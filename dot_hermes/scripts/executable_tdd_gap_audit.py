#!/usr/bin/env python3
"""Audit test coverage under a configurable projects directory.

Environment variables:
    TDD_GAP_PROJECTS_ROOT: repositories to scan (default: ~/projects)
    TDD_GAP_STATE_DIR: report directory (default: ~/.hermes/state)
    TDD_GAP_SKIP_DIRS: comma-separated directory names to skip
    TDD_GAP_EXEMPT_PROJECTS: comma-separated project names to mark exempt

Export any project-specific test variables before running the audit.
"""
...
PROJECTS_ROOT = Path(os.environ.get("TDD_GAP_PROJECTS_ROOT", Path.home() / "projects")).expanduser()
STATE_DIR = Path(os.environ.get("TDD_GAP_STATE_DIR", Path.home() / ".hermes" / "state")).expanduser()
SKIP_DIRS = set(filter(None, map(str.strip, os.environ.get("TDD_GAP_SKIP_DIRS", "").split(","))))
EXEMPT_PROJECTS = set(filter(None, map(str.strip, os.environ.get("TDD_GAP_EXEMPT_PROJECTS", "").split(","))))
...
def infer_runner(project_dir: Path, stack: str, test_files: list[str]) -> str:
    ...
    if stack == "node":
        ...
        # Fall back to pytest when tests exist without explicit runner configuration.
...
def _run_go_tests_per_package(project_dir: Path, env: dict) -> tuple[int, str]:
    ...
    Uses a temporary HOME to isolate test state.
    """
    import tempfile
...
def run_tests(project_dir: Path, runner: str) -> tuple[int, str]:
    ...
    # can legitimately exceed it on slower hosts.
    ...
    def _run_in_dir(cwd: Path) -> tuple[int, str]:
        run_env = env.copy()

        # Infer Django settings from any conventional WSGI module.
        if runner in ("pytest", "python -m pytest"):
            for wsgi in [*cwd.glob("*/wsgi.py"), cwd / "wsgi.py"]:
                if not wsgi.is_file():
                    continue
                try:
                    match = re.search(
...
def audit_project(project_dir: Path) -> dict:
    ...
    exemption_note = "configured exemption" if name in EXEMPT_PROJECTS else None
    if exemption_note:
        return {
            "name": name,
            "stack": stack,
